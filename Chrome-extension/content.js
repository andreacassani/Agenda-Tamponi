...
function getAppuntamenti(agenda, callback, retry) {
  ...
  try {
    const wInterfaces = document.documentElement.innerHTML.match(
    ...
    $.ajax({
      url: `/soleweb/?wicket:interface=:${wInterfaces[1]}${
        document.documentElement.innerHTML.match(/nuovaSchedaPanel/)
          ? ":nuovaSchedaPanel"
          : ""
      }:schedaPanel:formAppuntamento:${wInterfaces[2]}:IFormSubmitListener::`,
      method: "POST",
      data: formData,
      timeout: 30e3,
      success(data) {
        ...
        if (data.match(/prenotazioneCalendar::IBehaviorListener/)) {
          ...
          $.ajax({
            url: `/soleweb/?wicket:interface=:${wInterface}:prenotazioneCalendar::IBehaviorListener:0:&sid=${sid}&start=${
              getDate().a
            }&end=${getDate().b}&timezoneOffset=-60&anticache=${anticache()}`,
            method: "GET",
            dataType: "json",
            timeout: 15e3,
            success(dataAgenda) {
